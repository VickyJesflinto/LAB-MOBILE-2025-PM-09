...
public class MainActivity extends AppCompatActivity implements NoteAdapter.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addEditNoteLauncher = registerForActivityResult(
                ...
                result -> {
                    if (result.getResultCode() == RESULT_OK) {
                        loadNotes();
                        // Setelah memuat catatan, pastikan keyboard tidak muncul karena search bar
        ...
        // Panggil di akhir onCreate untuk setup awal
        preventKeyboardFromShowing();
    }
    ...
    protected void onResume() {
        super.onResume();
        // Panggil setiap kali activity kembali ke foreground
        preventKeyboardFromShowing();
    }

    private void preventKeyboardFromShowing() {
        if (editTextSearch != null) {
            editTextSearch.clearFocus(); // Hapus fokus dari search bar
        }

        // Minta fokus untuk elemen yang tidak bisa diedit (misalnya Toolbar)
        // Ini membantu mencegah EditText mendapatkan fokus secara otomatis.
        if (toolbarMain != null) {
            toolbarMain.requestFocus();
        } else {
            // Fallback jika toolbar null, coba root view
        ...
        // Secara eksplisit coba sembunyikan keyboard
        InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
        View currentFocusedView = getCurrentFocus(); // Dapatkan view yang MUNGKIN baru saja mendapatkan fokus
        if (imm != null) {
            if (currentFocusedView != null) {
                imm.hideSoftInputFromWindow(currentFocusedView.getWindowToken(), 0);
            } else {
                // Jika tidak ada view yang fokus, coba sembunyikan dari window utama
                // Ini mungkin diperlukan jika fokus belum sepenuhnya dialihkan
